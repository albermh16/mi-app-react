  ...
  */
  ...
  const Header = () => {
    ...
    const [categorias, setCategorias] = useState([]); // <-- variable state para organizar categorias principales
    ...
    //#endregion
    //#region -------- EFECTO DEL COMPONENTE -----------

    //#region -------- carga de categorias en navbar -----------
    // como recibimos las categorias principales en el LAYOUT desde el loader de App.jsx?
    // Hay dos formas:
    // 1- Usando el hook useLoaderData() importado de react-router-dom
    //const categorias = useLoaderData();
    // 2- usando efecto nada mas cargar el componente
    ...
    useEffect(() => {
      ...
      fetch('http://localhost:3000/api/Tienda/Categorias?pathCat=principales', { method: 'GET' })
        .then( async respuesta => {
          ...
          console.log(`respuesta del servidor a fetch categorias: ${JSON.stringify(bodyRespuesta)}`);
          setCategorias(bodyRespuesta.categorias);
          setLoading(false);
          setError(null);
          
        }).catch(error => {
          console.log(`error en fetch categorias: ${error.message}`);
          setCategorias([]); // vaciar categorias si hay error
          setLoading(false);
